...
public class DeletePlay implements Constants{
    private static View view;
    private static TextView textView;
    private static int deleteStatus;
    private static MainActivity mainActivity;

    public static void showDeleteCallConfirmDialog(final Context context, final Record callRecordItem, final int status) {
        deleteStatus = status;
    ...
    private static void deleteCall(Context context, Record callRecordItem) {
        try {
            FactoryHelper.getHelper().getDictaphoneRecordDAO().deleteItemAndFile(callRecordItem.getId());
            FragmentsHelper.updateAllLists(context);
